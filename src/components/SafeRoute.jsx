...
const SafeRoute = ({ source, destination }) => {
  ...
  const CRIME_JSON = "/crime_areas_latlong.json";
  ...
  // ---- Effect to initialize map and fetch data ----
  useEffect(() => {
    ...
    fetch(CRIME_JSON)
      ...
      .then(data => {
        ...
        calculateSafeRoute(); // recalc after crime data loads
      })
      .catch(err => {
  ...
  useEffect(() => {
    if (mapInstance.current) calculateSafeRoute();
    // eslint-disable-next-line
  }, [mode]);
  ...
  const calculateSafeRoute = async () => {
    ...
    try {
      ...
      try {
        const fastestRoute = await requestRoute(srcLat, srcLon, dstLat, dstLon, null, mode);
        const fastestRouteDuration = fastestRoute.features[0].properties.summary.duration;
        let safeRoute;
        if (avoidGeo) {
          safeRoute = await requestRoute(srcLat, srcLon, dstLat, dstLon, avoidGeo, mode);
        } else {
        ...
        const fallback = await requestRoute(srcLat, srcLon, dstLat, dstLon, null, mode);
  ...
  // Updated: include mode in API URL
  const requestRoute = async (srcLat, srcLon, dstLat, dstLon, avoidGeo = null, mode = "driving-car") => {
    const body = {
  ...
  return (
    <div>
      <div style={{ marginBottom: 12, textAlign: "center" }}>
        <label htmlFor="mode-select" style={{ color: "#fff", marginRight: 12 }}>Travel Mode:</label>
        <select
          id="mode-select"
          value={mode}
          onChange={e => setMode(e.target.value)}
          style={{ borderRadius: '7px', padding: '5px 15px', fontSize: '1em', background: '#111', color: '#fff', border: '1px solid #888' }}
        >
